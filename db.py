"""
db.py — SQLite database helpers for Watt's Off.

Provides a connection factory and schema initializer.
Uses wattsoff.db in the project root directory (local) or /tmp (Vercel).
"""

import sqlite3
import os

# Vercel's filesystem is read-only except /tmp. Detect Vercel and use /tmp.
...
def reset_db(db_path=None):
    """
    Drops and recreates the database file. Used by simulate.py --seed
    to start fresh.
    """
    path = db_path or DB_PATH
